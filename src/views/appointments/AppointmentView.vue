<template>
    <h2 class="text-2xl lg:text-4xl font-extrabold text-white">Detalles Cita y Resumen</h2>
    <p class="text-white text-lg mt-5">Verifica la informacion y confirma tu cita</p>
    <h3 class="text-2xl lg:text-3xl font-extrabold text-white">Servicios</h3>
    <p 
    ...
    <div class="space-y-4 lg:space-y-4" v-if="!appointments.noServicesSelected">
        ...
        <div class="lg:flex gap-5 items-start">
            ...
            >
                ...
                <div v-if="appointments.isValidReservation" class="flex justify-end col-span-1 lg:col-span-2" ref="confirmButtonRef">
                    <button 
                        class="w-full bg-green-500 hover:bg-green-700 my-5 lg:mt-0 p-3 rounded-lg uppercase font-black text-white"
                        :class="{'bg-green-700' : appointments.isSavingAppointment}"
                        @click="appointments.saveAppointment"
                        :disabled="appointments.isAppointmentConfirmed"
                    >
                        {{ appointments.isSavingAppointment ? 'Guardando turno...' : 'Confirmar turno' }}
                    </button>
                </div>
            </div>    
        </div>   
...
<script setup>
    ...
    const onDateSelect = async() => {
        await nextTick()
        hoursDiv.value.scrollIntoView({ behavior: 'smooth' })
    }
    
    const scrollToConfirmButton = async () => {
        await nextTick()
        confirmButtonRef.value.scrollIntoView({ behavior: 'smooth' })
    } 
</script>